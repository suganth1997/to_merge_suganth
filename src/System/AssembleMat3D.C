...
void TAssembleMat3D::Assemble3D()
{
    ...
    for (i = 0; i < N_Cells; i++)
    {
        ...
        if ((TDatabase::ParamDB->DISCTYPE == SDFEM) || (TDatabase::ParamDB->BULK_REACTION_DISC == SDFEM) || (TDatabase::ParamDB->CELL_MEASURE == 4) || (TDatabase::ParamDB->TURBULENT_VISCOSITY_TYPE == 105) || (TDatabase::ParamDB->TURBULENT_VISCOSITY_TYPE == 108) || (TDatabase::ParamDB->TURBULENT_VISCOSITY_TYPE == 109))
        {
...
//========================================================================
#ifdef _UPDATED_LAGRANGIAN
        if (N_Rhs)
            this->AddLocalRhsToGlobalDeformed(i, cell, N_BaseFunct, BaseFuncts, reftrans);
#else
        if (N_Rhs)
        ...
        for (m = 0; m < N_; m++)
        {
            ...
            if (l < ActiveBound)
            {
                ...
                for (k = 0; k < N_; k++)
                {
                    ...
                    if (l2 == 0)
                    {
                        cout << l << "(assemble3d.c) not found :: " << RowPtr[l] << " j :: " << j << " end :: " << end << endl;
                        exit(0);
                    }
                } // endfor k
            }     // endif l
...
void TAssembleMat3D::AddLocalRhsToGlobalDeformed(int i, TBaseCell *cell, int *N_BaseFunct, BaseFunct3D *BaseFuncts, RefTrans3D reftrans)
{
    int j, k, l, m, l3, N_, l1, dof, ActiveBound, DirichletBound, *DOF, N_H;
    int comp, N_EdgeDOF, N_Joints, *JointDOF, *EdgeDOF, N_Edges, N_VertInCell;
    const int *TmpFV, *TmpLen, *EdgeVertex, *TmpFE, *ETmpLen;
    ...
    double FunctionalValues[MaxN_BaseFunctions3D];
    double AbsDetjk[MaxN_QuadPoints_3D];
    double **JointValues, *JointValue, t1, t2, **JointDerivatives, *JointDerivative;

    std::unique_ptr<std::vector<Eigen::Matrix3d>[]> cof_F(new std::vector<Eigen::Matrix3d>[6]);
        ...
        for (m = 0; m < N_Joints; m++)
        {
            ...
            if (InnerBoundary || OuterBoundary)
            {
                ...
                case ROBIN: // lhs of robin will be assembled in main program
                    ...
                    Find ux, uy, uz
                         vx, vy, vz
                         wx, wy, wz for all quad points of the joint

                    */

                    double *sol = Rhs[0] + 3 * fespace->GetN_DegreesOfFreedom();

                    if (!j)
                    { // Calculate uvw values only once
                        cof_F[m].resize(N_Points);

                        double **JointDerivatives_X = TFEDatabase3D::GetJointDerivatives3D(BaseFuncts[CurrentElement], FaceQuadFormula, m, D100);
                        double **JointDerivatives_Y = TFEDatabase3D::GetJointDerivatives3D(BaseFuncts[CurrentElement], FaceQuadFormula, m, D010);
                        double **JointDerivatives_Z = TFEDatabase3D::GetJointDerivatives3D(BaseFuncts[CurrentElement], FaceQuadFormula, m, D001);

                        int *dof = fespace->GetGlobalDOF(i);
                        int N_U = fespace->GetN_DegreesOfFreedom();

                        for (int i_quad = 0; i_quad < N_Points; i_quad++)
                        {
                            double ux = 0.0, uy = 0.0, uz = 0.0;
                            double vx = 0.0, vy = 0.0, vz = 0.0;
                            double wx = 0.0, wy = 0.0, wz = 0.0;

                            for (int i_Basis = 0; i_Basis < N_; i_Basis++){
                                ux += sol[dof[i_Basis]] * JointDerivatives_X[i_quad][i_Basis];
                                vx += sol[dof[i_Basis] + N_U] * JointDerivatives_X[i_quad][i_Basis];
                                wx += sol[dof[i_Basis] + 2*N_U] * JointDerivatives_X[i_quad][i_Basis];

                                uy += sol[dof[i_Basis]] * JointDerivatives_Y[i_quad][i_Basis];
                                vy += sol[dof[i_Basis] + N_U] * JointDerivatives_Y[i_quad][i_Basis];
                                wy += sol[dof[i_Basis] + 2*N_U] * JointDerivatives_Y[i_quad][i_Basis];

                                uz += sol[dof[i_Basis]] * JointDerivatives_Z[i_quad][i_Basis];
                                vz += sol[dof[i_Basis] + N_U] * JointDerivatives_Z[i_quad][i_Basis];
                                wz += sol[dof[i_Basis] + 2*N_U] * JointDerivatives_Z[i_quad][i_Basis];
                            }
                            double a[3][3];
                            a[0][0] = ux + 1;
                            a[0][1] = uy;
                            ...
                            for (int i_cof = 0; i_cof < 3; i_cof++){
                                for (int j_cof = 0; j_cof < 3; j_cof++){
                                    cof_F[m][i_quad](i_cof, j_cof) = a[(i_cof + 1) % 3][(j_cof + 1) % 3] * a[(i_cof + 2) % 3][(j_cof + 2) % 3] -
                                                                     a[(i_cof + 1) % 3][(j_cof + 2) % 3] * a[(i_cof + 2) % 3][(j_cof + 1) % 3];
                                }
                            }
                        }
                    }
                    ...
                    case 4:
                        ...
                        for (l = 0; l < N_Points; l++)
                        {
                            ...
                            double mod_n0 = sqrt(t1);
                            double n0_x = nx / mod_n0, n0_y = ny / mod_n0, n0_z = nz / mod_n0;

                            /* 

                            Find cof(F)*n0 for the component and store it in TDatabase NTi

                            */
                            
                            TDatabase::ParamDB->NT1 = cof_F[m][l](j, 0) * n0_x + cof_F[m][l](j, 1) * n0_y + cof_F[m][l](j, 2) * n0_z;
