...
TSystemTNSE3D::TSystemTNSE3D(int N_levels, TFESpace3D **velocity_fespace, TFESpace3D **presssure_fespace, TFEVectFunct3D **velocity, 
...
void TSystemTNSE3D::Init(CoeffFct3D *lincoeffs, BoundCondFunct3D *BoundCond, BoundValueFunct3D *U1BoundValue, 
...
//           case CLASSICAL_LES:    
//             DiscreteFormARhs = DiscreteFormClassicalLES;
//             DiscreteFormNL = DiscreteFormNLClassicalLES;
//             DiscreteFormRhs = DiscreteFormRHS; 
//             break;
      ...
      switch(Disctype)
       {
        ...
        case VMS_PROJECTION_EXPL:
#ifdef __PRIVATE__  
            NSEaux[i] =  new TAuxParam3D(TimeNSN_FESpacesVelo_GradVelo_VMS3D, 
					 TimeNSN_FctVelo_GradVelo_VMS3D,
					 TimeNSN_ParamFctVelo_GradVelo_VMS3D,
                                         TimeNSN_FEValuesVelo_GradVelo_VMS3D,
					 fesp_aux, fefct_aux+(i*6),
					 TimeNSFctVelo_GradVelo_VMS3D,
					 TimeNSFEFctIndexVelo_GradVelo_VMS3D,
                                         TimeNSFEMultiIndexVelo_GradVelo_VMS3D,
					 TimeNSN_ParamsVelo_GradVelo_VMS3D,
					 TimeNSBeginParamVelo_GradVelo_VMS3D);
#endif 
        break; 
     ...
     if(i == N_Levels-1)
      {
       ...
       RhsOnlyAssemble  = new TAssembleMat3D(N_FESpaces, fesp, N_SquareMatrices, NULL, N_RectMatrices, NULL,
                              N_Rhs, RHSs, fesprhs, DiscreteFormRhs, BoundaryConditions, BoundaryValues, NSEaux[i]);
...
//  cout << " TSystemTNSE3D::Init done ! " << endl; 
//          exit(0);
  ...
  double alpha[2];
   ...
   for(i=Start_Level;i<N_Levels;i++)
    {     
      ...
      //these mattrices are not part of AMatRhsAssemble, so reset here
...
//       if(Disctype == VMS_PROJECTION)
//          {
//           Matrices_tilde_G11[i]->Reset();
//           Matrices_tilde_G22[i]->Reset();
//           Matrices_tilde_G33[i]->Reset();
//          }  
// #endif
...
void TSystemTNSE3D::AssembleSystMat(double scale, double *oldrhs, double *rhs, double *sol)
{
 int i;
 double tau, val = TDatabase::TimeDB->SCALE_DIVERGENCE_CONSTRAINT;
  ...
  switch(NSEType)
   ...
   memcpy(B+N_Active, RhsArray[i]+N_Active, N_DirichletDof*SizeOfDouble);
   memcpy(B+N_U+N_Active, RhsArray[i]+N_U+N_Active, N_DirichletDof*SizeOfDouble); 
   memcpy(B+2*N_U+N_Active, RhsArray[i]+2*N_U+N_Active, N_DirichletDof*SizeOfDouble);     
...
void TSystemTNSE3D::GetResidual(double *sol, double &impuls_residual, double &residual)
{
#ifdef _MPI   
...
void TSystemTNSE3D::MeasureTNSEErrors(DoubleFunct3D *ExactU1, DoubleFunct3D *ExactU2,  DoubleFunct3D *ExactU3, DoubleFunct3D *ExactP, double *AllErrors)
{
  int i;
  double errors[9];
...
void TSystemTNSE3D::GetMesh()
{
  ...
  int MaxLen;
  int N_Points;
  int xmax=-1,ymax=-1,zmax=-1,xmin=1,ymin=1,zmin=1;
  int n = coll->GetN_Cells();
  
  double xc,yc,zc;
  
  std::ofstream myfile;
/*  
  if(TDatabase::ParamDB->OLD==0)
    myfile.open("fromgmesh.mesh");
  else*/
    myfile.open("togmesh.mesh");
  
// reset all vertices faces and edges to -1  
  for(int i=0;i<n;i++){
    cell = coll->GetCell(i);
    ...
    for(int j=0;j<cell->GetN_Joints();j++)
      cell->GetJoint(j)->SetClipBoard(-1);
  ...
  for(int i=0;i<n;i++){
    ...
    for(int j=0;j<cell->GetN_Vertices();j++){
      clip = cell->GetVertex(j)->GetClipBoard();
      if(clip==-1){
	cell->GetVertex(j)->SetClipBoard(count);
  ...
  for(int i=0;i<n;i++){
    ...
    for(int j=0;j<cell->GetN_Joints();j++){
      ...
      clip =  cell->GetJoint(j)->GetClipBoard();
      
      if(clip==-1 && (cell->GetJoint(j)->GetType() == BoundaryFace || cell->GetJoint(j)->GetType() == IsoBoundFace)){
       cell->GetJoint(j)->SetClipBoard(count);
       count++;

	N_Points = TmpLen[j];        
        for(int k=0;k<N_Points;k++)
        {         
          clip = cell->GetVertex(TmpFV[j*MaxLen+k])->GetClipBoard();
	//  myfile << " " << clip+1;
	}
	    xc=yc=zc=0;
	for(int k=0;k<N_Points;k++)
...
void TSystemTNSE3D::CheckAllMat()
{
        char key[10000];
      ...
      sprintf(key,"SqmatrixM11");
      DOF_stats((TMatrix3D*)SqmatrixM11[N_Levels-1],'U',0,key);
      sprintf(key,"SqmatrixM22");
      DOF_stats((TMatrix3D*)SqmatrixM22[N_Levels-1],'U',1,key);
      sprintf(key,"SqmatrixM33");
      DOF_stats((TMatrix3D*)SqmatrixM33[N_Levels-1],'U',2,key);

#ifdef __PRIVATE__
 if(TDatabase::ParamDB-> DISCTYPE==9){  
            sprintf(key,"Matrices_tilde_G11");
      DOF_stats((TMatrix3D*)Matrices_tilde_G11[N_Levels-1],'U',0,key);
      
            sprintf(key,"Matrices_tilde_G22");
      DOF_stats((TMatrix3D*)Matrices_tilde_G22[N_Levels-1],'U',1,key);
      
            sprintf(key,"Matrices_tilde_G33");
      DOF_stats((TMatrix3D*)Matrices_tilde_G33[N_Levels-1],'U',2,key);
      
            sprintf(key,"Matrices_G11");
      DOF_stats((TMatrix3D*)Matrices_G11[N_Levels-1],'V',0,key);
      
            sprintf(key,"Matrices_G22");
      DOF_stats((TMatrix3D*)Matrices_G22[N_Levels-1],'V',1,key);
      
            sprintf(key,"Matrices_G33");
      DOF_stats((TMatrix3D*)Matrices_G33[N_Levels-1],'V',2,key);
 }
#endif  
...
void TSystemTNSE3D::DOF_stats(TMatrix3D* MatB, char M , int k,char * name)
{
...
if(M == 'U')
  ...
  fspace = Projection_Spaces[N_Levels-1];
else
...
//  sprintf(str,"All_Matrices/%s.txt",name);
   sprintf(str,"Galerkin/%s.txt",name);
   else
     sprintf(str,"VMS/%s.txt",name);
#endif
...
//cout << "No. of rows are :: "<< MatB->GetN_Rows() << endl;
...
#else
 if(TDatabase::ParamDB->DISCTYPE==1)
 sprintf(str,"Galerkin/rhs.txt");
 else
   sprintf(str,"VMS/rhs.txt");
#endif
